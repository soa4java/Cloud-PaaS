...
import com.primeton.paas.app.mail.ISystemConfig;

/**
 * 
 * @author liuyi (mail-to:liu-yi@primeton.com)
 */
public class MailConfig implements ISystemConfig{
	...
	private static ILogger logger = LoggerFactory.getLogger(MailConfig.class);
	
	/*
	 * (non-Javadoc)
	 * @see com.primeton.paas.app.mail.ISystemConfig#getQueueGroup()
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see com.primeton.paas.app.mail.ISystemConfig#getMQServer()
	 */
	...
	public MQServer getMQServer() {
		if(this.server == null) {
			...
			if(var != null) {
				...
				if(val != null && val.trim().length() > 0) {
					try {
						Map<String, String> args = (Map<String, String>)JsonSerializerUtil.deserialize(val);
						server = new MQServer();
