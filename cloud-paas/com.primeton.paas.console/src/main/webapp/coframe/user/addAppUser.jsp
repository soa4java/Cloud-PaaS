...
<head>
    <title><%=NLSMessageUtils.getMessage("new.user") %></title>
...
<body>    
    <form id="form1" method="post" action="<%=request.getContextPath() %>/srv/authuser/addAppUser">
        ...
        <div style="padding-left:11px;padding-bottom:5px;">
            <table style="table-layout:fixed;" border="0">
                <tr>
                    ...
                    <td style="width:45%;">    
                         <input name="userId" class="nui-textbox" onvalidation="onUserIdValidation" width="80%"/><!-- </span> * (登录名)长度为4-15位，以字母开头、允许数字和下划线 -->
                    </td>
                    <td>*4-15位，以字母开头、允许数字和下划线 </td>
                </tr>
                 <tr>
    ...
    <script type="text/javascript">
        ...
        function onUserIdValidation(e){
        	//var error = document.getElementById("userId_error");
        	if(!e.value){
        		e.errorText="不能为空！";
                ...
                $ .ajax({
                	url: "<%=request.getContextPath() %>/srv/authuser/checkUserId/" +userId,
                	async: false,
        ...
        function saveData() {
            ...
            form.loading("操作中，请稍后......");
            $.ajax({
                url: "<%=request.getContextPath() %>/srv/authuser/addAppUser",
                type: "post",
                data: { keyData: json },
                success: function (text) {
                	var o = nui.decode(text);
