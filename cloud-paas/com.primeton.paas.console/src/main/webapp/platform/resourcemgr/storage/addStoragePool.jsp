...
<body>
	<form id="form1" method="post" style="font-size:12px; " >
  	   <input id="id" name="id" value="<%=id%>" class="nui-hidden" />
       <fieldset style="border:solid 1px #aaa;padding:3px;">
            <legend >存储池当前配置</legend>
        <div style="padding:5px;">
	       	  <table>
	            <tr>
	                <td >启用监视器:</td>
	                <td >    
	                    <div id="isEnable" name="isEnable" trueValue="1" falseValue="0" class="nui-checkbox" checked="true" readOnly="false"></div>
	                </td>
	            </tr> 
	            <tr>
	            	<td>容量(G):</td>
	            	<td id="selId">
	            		<input id="storageSize" name="storageSize" class="nui-combobox"  style="width:150px;"
	            			textField="value" valueField="value"  
						    url="<%=request.getContextPath() %>/srv/storagePoolMgr/getFilterAppStorageSizes"
	            		/>
	            	</td>
	            </tr>
	            <tr>
	            	...
	            	<td >
	            		<input id="increaseSize" name="increaseSize" class="nui-combobox"  style="width:150px;"
	            			textField="value" valueField="value"  
						    url="<%=request.getContextPath() %>/srv/storagePoolMgr/getStoragePoolIncreaseSize"
	            		/>
	            	</td>
	            </tr>
	            <tr>
	            	...
	            	<td >
	            		<input id="decreaseSize" name="decreaseSize" class="nui-combobox"  style="width:150px;"
	            			textField="value" valueField="value"  
						    url="<%=request.getContextPath() %>/srv/storagePoolMgr/getStoragePoolDecreaseSize"	            		
	            		/>
	            	</td>
	            </tr>
	            <tr>
	            	...
	            	<td >
	            		<input id="maxSize" name="maxSize" class="nui-combobox"  style="width:150px;"
	            			textField="value" valueField="value"  
						    url="<%=request.getContextPath() %>/srv/storagePoolMgr/getStoragePoolMaxSize"	
	            ...
	            <tr>
	            	...
	            	<td >
	            		<input id="minSize" name="minSize" class="nui-combobox"  style="width:150px;"
	            			textField="value" valueField="value"  
						    url="<%=request.getContextPath() %>/srv/storagePoolMgr/getStoragePoolMinSize"	
	            ...
	            <tr>
	            	...
	            	<td >
	            		<input id="timeInterval" name="timeInterval" class="nui-combobox"  style="width:150px;"
	            			textField="value" valueField="value"  
						    url="<%=request.getContextPath() %>/srv/storagePoolMgr/getStoragePoolTimeInterval"	
	            		/>
	            	</td>
	            </tr>	
	            <tr>
	  ...
	  <div style="text-align:center;padding:10px;">      
	  	   <span id="changeInfo-Add">         
	       		<a class="nui-button" onclick="addStoragePool" style="width:60px;margin-right:20px;">添加</a>     
				<a class="nui-button" onclick="onCancel" style="width:60px;">取消</a>      
	       </span>      
	  	   <span id="changeInfo-Update" style="display:none">         
	       		<a class="nui-button" onclick="updateStoragePool" style="width:60px;margin-right:20px;">修改</a>      
	       		<a class="nui-button" onclick="onCancel" style="width:60px;">取消</a>      
	       </span>      
       </div>   
  </form>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        nui.getbyName("storageSize").select(0);
    ...
    function SetData(data) {
        if (data.action == "details") {
            ...
            id = data.id;
            if (id != null) {
            	document.getElementById("selId").innerHTML = data.storageSize +'G'+'<input id="storageSize" name="storageSize" class="nui-hidden"  value="'+data.storageSize+'"/>';
                nui.parse();
            	...
            	nui.getbyName("storageSize").setUrl('<%=request.getContextPath() %>/srv/storagePoolMgr/getFilterAppStorageSizes');
            }
        }
    }
    ...
    function addStoragePool() {
       ...
       if (form.isValid() == false) {
		...
		$.ajax({
        	type: "PUT",
            url: "<%=request.getContextPath() %>/srv/storagePoolMgr/addStoragePool",
    ...
    function updateStoragePool() {
		...
		nui.mask({
 		...
 		$.ajax({
         	type: "PUT",
             url: "<%=request.getContextPath() %>/srv/storagePoolMgr/updateStoragePool",
             contentType: "application/json; charset=utf-8",
             data: json,
             success: function (text) {
             	if(text){
