...
<body>
	<form id="form1" method="post" action="<%=request.getContextPath() %>/srv/service/getCardBinClusterDetail">
       	...
       	<fieldset style="border:solid 1px #aaa;padding:3px;">
            <legend >集群扩展属性</legend>
            <div style="padding:5px;">
		        <table style="width:100%">
		             <tr>
	                    <td style="width:10%;"  align="right">数据库连接字符串：</td>
	                    <td style="width:80%;">    
	                         <input name="attributes.jdbcUrl" class="nui-textbox asLabel" width="100%"/>
	                    </td>
                	</tr>
	                <tr>
	                ...
	                <tr>
	                    <td style="width:10%;"  align="right">数据库用户：</td>
	                    <td style="width:80%;">    
	                         <input name="attributes.jdbcUser" class="nui-textbox asLabel" width="100%"/>
	                    </td>
	                </tr>
	                <tr>
	                    <td style="width:10%;"  align="right">数据库连接池个数：</td>
	                    <td style="width:80%;">    
	                    	<input name="attributes.jdbcMinPoolSize" class="nui-textbox asLabel" width="2%"/>-&nbsp;<input name="attributes.jdbcMaxPoolSize" class="nui-textbox asLabel" width="40%"/>
	                    </td>
	                </tr>        
		        </table>            
            </div>
        ...
        <fieldset style="border:solid 1px #aaa;padding:3px;">
            ...
            <div style="padding:5px;">
		        <table style="width:100%">
		             <tr>
	                    <td style="width:18%;"  align="right">【数据同步】 - 本地临时目录：</td>
	                    <td style="width:80%;">    
	                         <input name="attributes.tempFilePath" class="nui-textbox asLabel" width="100%"/>
	                    </td>
                	</tr>
	                <tr>
	                    <td style="width:18%;"  align="right">【数据同步】 - 本地保存目录：</td>
	                    <td style="width:80%;">    
	                         <input name="attributes.destFilePath" class="nui-textbox asLabel" width="100%"/>
	                    </td>
	                </tr>
	                <tr>
	                    <td style="width:18%;"  align="right">【数据同步】 - 数据同步时间：</td>
	                    <td style="width:80%;">    
	                         每月【<input name="attributes.syncDay" class="nui-textbox asLabel" width="2%"/>】号
	                         【<input name="attributes.syncHour" class="nui-textbox asLabel" width="2%"/>】时 (24小时制)
	                    </td>
	                </tr>
		        </table>            
            </div>
	...
	<div id="datagrid" class="nui-datagrid" style="width:100%;height:80px" allowResize="true"
         idField="id" multiSelect="true" url="<%=request.getContextPath() %>/srv/service/cardbinService/<%=clusterId %>" showPager="false" onloaderror="onLoadErrorRenderer">
        <div property="columns">
            <!-- <div type="checkcolumn" ></div> -->        
            <div field="id" width="100" headerAlign="center" align ="center" allowSort="false" renderer="onActionRenderer">服务标识</div>    
            <div field="name" width="100" headerAlign="center" align ="center" allowSort="false">服务名称</div>    
    ...
    <script type="text/javascript">        
        ...
        function initForm() {
        	...
        	$.ajax({
                url: "<%=request.getContextPath() %>/srv/service/getCardBinClusterDetail/" +clusterId,
        ...
        function onMemorySizeActionRenderer(e) {
	        var grid = e.sender;
	        var record = e.record;
	        return record.minMemorySize + 'M - ' + record.maxMemorySize + "M";
        }
        ...
        function goBack() {
        	window.location = "cardbinClusterMgr.jsp";
        }
        
        function configCardBinService(serviceId){
        	var clusterId = nui.get("id").value;
        	nui.open({
                url: "<%=request.getContextPath() %>/platform/srvmgr/cardbin/configCardBin.jsp",
                title: "CardBin服务更新向导", width: 850, height: 600,
                onload: function () {
                    var row = grid.getSelected();
                    var iframe = this.getIFrameEl();
                    var data = { clusterId:clusterId, serviceId:serviceId };
        ...
        function removeService(serviceId) {
        	...
        	$.ajax({
				url: "<%=request.getContextPath() %>/srv/service/removeCardBinService/" + serviceId,
                success: function (text) {
                	nui.unmask();
                	var o = nui.decode(text);
                	if (o.result == true) { 
