...
<%@page import="com.primeton.paas.manage.api.service.JobCtrlService"%>
...
<body>         
	<form id="form1" method="post" action="<%=request.getContextPath() %>/srv/service/createJobCtrl">
       	<fieldset style="border:solid 1px #aaa;padding:3px;">
            <legend >新增JobCtrl服务向导</legend>
            <div style="padding:5px;">
            	<input name="order.orderType" value="CreateJobCtrl" class="nui-hidden" />
            	<input name="order.itemList[0].itemType" value="<%=JobCtrlService.TYPE %>" class="nui-hidden" />
		        <table border="0">
		            <tr>
		            ...
		            <tr>
	                    ...
	                    <td>    
	                    	 ...
	                    	 <input name="order.itemList[0].attrList[1].attrValue" class="nui-textbox" onvalidation="onNullValidation" value="<%=JobCtrlService.TYPE %> Service" width="80%"/>
	                    </td>
	                    <td>*服务显示名称，与业务无关。</td>
	                </tr>
		            <tr>
	                ...
	                <tr>
	                    ...
	                    <td>    
	                    	 ...
	                    	 <input name="order.itemList[0].attrList[3].attrValue"  class="nui-combobox" data='<%=databaseTypesJSON %>' value="Oracle" width="80%"/>
	                    </td>
	                    <td>* 数据库类型</td>
	                </tr>
	                <tr>
	                ...
	                <tr>
	                    <td align="right">调度器日志保存时长：</td>
	                    <td>    
	                         <input name="order.itemList[0].attrList[10].attrName" value="<%=OrderItemAttr.ATTR_SCHEDULER_SAVE_DAYS%>" class="nui-hidden" />
	                    	 <input name="order.itemList[0].attrList[10].attrValue"  class="nui-textbox" value="7" width="80%" required="true" onvalidation="onNullValidation" />
	                    </td>
	                    <td>* 调度器日志保存时长（单位：天）</td>
	                </tr>
	                <tr>
	                    <td align="right">作业流执行时间范围：</td>
	                    <td>    
	                         <input name="order.itemList[0].attrList[11].attrName" value="<%=OrderItemAttr.ATTR_SCHEDULER_TIME_ROUND%>" class="nui-hidden" />
	                    	 <input name="order.itemList[0].attrList[11].attrValue"  class="nui-textbox" value="8:40-9:50" width="80%" required="true" onvalidation="onNullValidation" />
	                    </td>
	                    <td>* 作业流执行时间范围（必须是整点）</td>
	                </tr>
		        </table>            
            </div>
        </fieldset>
    ...
    <script type="text/javascript">        
        ...
        function onAdd() {
        	...
        	if (!confirm("你确定要提交JobCtrl服务创建申请？")) {
    	        return false;
    	    }
            ...
            $.ajax({
				url: "<%=request.getContextPath() %>/srv/service/createJobCtrl",
