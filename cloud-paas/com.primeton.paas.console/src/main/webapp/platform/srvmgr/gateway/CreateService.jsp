...
<body>         
	<form id="form1" method="post" action="<%=request.getContextPath() %>/srv/service/createGateway">
       	<fieldset style="border:solid 1px #aaa;padding:3px;">
            <legend >新增网关服务向导</legend>
            <div style="padding:5px;">
            	<input name="order.itemList[0].itemType" value="<%=GatewayService.TYPE %>" class="nui-hidden" />
            	<input name="order.orderType" value="CreateGateway" class="nui-hidden" /> <!-- 创建简单的gateway服务集群及其服务实例 -->
		        <table border="0">
		            <tr>
	                ...
	                <tr>
	                    <td align="right">集群大小：</td>
	                    <td>    
	                         <input name="order.itemList[0].attrList[3].attrName" value="<%=OrderItemAttr.ATTR_CLUSTER_SIZE%>" class="nui-hidden" />
	                    	 <input name="order.itemList[0].attrList[3].attrValue"  class="nui-combobox" data='<%=gatewaySizeJSON %>' value="1" width="80%"/>
	                    </td>
	                    <td>* 1(主节点)+N(复制节点)</td>
	                </tr>
	                <tr>
	                    ...
	                    <td>    
	                         <input name="order.itemList[0].attrList[4].attrName" value="<%=OrderItemAttr.ATTR_GATEWAY_MAX_CONNECTION%>" class="nui-hidden" />
	                    	 <input name="order.itemList[0].attrList[4].attrValue"  class="nui-combobox" data='<%=gatewayMaxConnectionJSON %>' value="100000" width="80%"/>
	                    </td>
	                    <td>* 最大连接数(终端数)</td>
	                </tr>
	                <tr>
	                    ...
	                    <td>    
	                         <input name="order.itemList[0].attrList[5].attrName" value="<%=OrderItemAttr.ATTR_GATEWAY_PRE_SERVERS%>" class="nui-hidden" />
	                    	 <input name="order.itemList[0].attrList[5].attrValue" class="nui-textbox" onvalidation="onNullValidation" value="127.0.0.1:8888" width="80%"/>
	                    </td>
    ...
    <script type="text/javascript">        
        ...
        function onAdd() {
            ...
            $.ajax({
				url: "<%=request.getContextPath() %>/srv/service/createGateway",
