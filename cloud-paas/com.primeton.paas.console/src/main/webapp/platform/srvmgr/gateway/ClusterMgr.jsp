...
<body style="height:80%">         
    ...
    <div id="datagrid" class="nui-datagrid" style="width:99%;height:100%" allowResize="true"
         idField="id" multiSelect="true" url="<%=request.getContextPath() %>/srv/service/gatewayCluster" showPager="true" onloaderror="onLoadErrorRenderer">
    ...
    <script type="text/javascript">        
        ...
        function removeCluster(clusterId, type) {
        	...
        	$.ajax({
            	url : "<%=request.getContextPath() %>/srv/service/removeGatewayService",
		...
		function createCluster() {
			nui.open({
                url: "<%=request.getContextPath() %>/platform/srvmgr/gateway/CreateService.jsp",
                title: "新增网关服务向导", width: 800, height: 300,
                onload: function () {
                    //nothing
                },
		...
		function update() {
			...
			if (rows.length != 1) {
				nui.alert("请选择一个网关集群.", "系统提示");
				return;
			}
			var cluster = rows[0].id;
			// alert(cluster);
			nui.open({
                url: "<%=request.getContextPath() %>/platform/srvmgr/gateway/UpdateService.jsp?cluster=" + cluster,
                title: "修改网关服务向导:" + cluster, width: 800, height: 300,
                onload: function () {
                    //nothing
                },
                ondestroy: function (action) {
