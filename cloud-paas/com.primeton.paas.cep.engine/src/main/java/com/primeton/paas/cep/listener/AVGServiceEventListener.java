 ...
 *
 */
public class AVGServiceEventListener extends AbstractEventListener implements Constants{
	
	private static final String MSG_TYPE_KEY = "MSG_TYPE";
	private static final String MSG_TYPE_VALUE = "AVG_SERVICE_MONITOR";
	
	private ServerContext context = ServerContext.getContext();
	private static final String EXCHANGE_NAME = "avgServiceMonitorExchange";
	private boolean isCreated = false;
	
	private static ILogger logger = LoggerFactory.getLogger(AVGServiceEventListener.class);
	
	public static final String EPS = "select clusterId, avg(cpu_us), avg(cpu_sy), avg(cpu_ni), avg(cpu_id), avg(cpu_wa), avg(cpu_hi), avg(cpu_si), avg(cpu_st), avg(cpu_oneload), avg(cpu_fiveload), avg(cpu_fifteenload), avg(mem_total), avg(mem_used), avg(mem_free), avg(mem_buffers), avg(mem_us), avg(io_si), avg(io_so), avg(io_bi), avg(io_bo) from serviceMonitorEvent(100-cpu_us>=0, 100-cpu_sy>=0, 100-cpu_ni>=0, 100-cpu_id>=0, 100-cpu_wa>=0, 100-cpu_hi>=0, 100-cpu_si>=0, 100-cpu_st>=0, 100-mem_us>=0).win:time_batch(? sec) where clusterId = ? group by clusterId";
	...
	public void update(EventBean[] newEvents, EventBean[] oldEvents) {
		...
		String clusterId = getValue(e,"clusterId");
		if(StringUtil.isEmpty(clusterId)|| e.get("avg(cpu_us)") == null){
			return;
		}
		...
		message.putHeader(CLUSTER_ID, clusterId);//TODO
