 ...
 *
 */
public class EsbConsoleServiceManager extends DefaultServiceManager 
		implements IEsbConsoleServiceManager {

	public static final String TYPE = EsbConsoleService.TYPE;
	...
	static {
		drivers.put(EsbConsoleService.DATABASE_MySQL, "com.mysql.jdbc.Driver"); //$NON-NLS-1$
		drivers.put(EsbConsoleService.DATABASE_ORACLE, "oracle.jdbc.driver.OracleDriver"); //$NON-NLS-1$
		drivers.put(EsbConsoleService.DATABASE_DB2, "com.ibm.db2.jcc.DB2Driver"); //$NON-NLS-1$
	}
	
	/**
	 * Default. <br>
	 */
	public EsbConsoleServiceManager() {
		super();
	}
	...
	public void start(String id) throws ServiceException {
		...
		EsbConsoleService service = getServiceQuery().get(id, EsbConsoleService.class);
		if (null == service) {
	 ...
	 * @param service
	 */
	protected void beforeStart(EsbConsoleService service) {
		if (null == service) {
			return;
		}
		// 修改Tomcat ${catlina.home}/conf/server.xml文件
		{
			...
			WriteFileMessage message = new WriteFileMessage(content, SystemVariables.getProgramsHome() 
					+ "/EsbConsole/apache-tomcat-6.0.44/conf/server.xml"); //$NON-NLS-1$
			...
			for (int i=0; i<5; i++) {
				try {
					...
					logger.error(null == result ? StringUtil.format("No response while send modify EsbConsole service {0} configuration file server.xml on host {1}.", 
		...
		// 修改Tomcat ${catlina.home}/conf/context.xml文件
		{
			...
			WriteFileMessage message = new WriteFileMessage(content, SystemVariables.getProgramsHome() 
					+ "/EsbConsole/apache-tomcat-6.0.44/conf/context.xml"); //$NON-NLS-1$
			message.setEncoding(StringUtil.CHARSET_UTF8);
			message.setNeedResponse(true);
			...
			for (int i=0; i<5; i++) {
				try {
					...
					logger.error(null == result ? StringUtil.format("No response while send modify EsbConsole service {0} configuration file context.xml on host {1}.", 
							new Object[] { service.getId(), service.getIp() }) : result.getBody());
				} catch (MessageException e) {
					logger.error(e);
				}
	 ...
	 * @return
	 */
	private String getContextConfig(EsbConsoleService service) {
		if (null == service) {
			return null;
		}
		...
		InputStream in = null;
		try {
			in = EsbSAMServiceManager.class.getResourceAsStream("/META-INF/templates/ESB/console/context.xml"); //$NON-NLS-1$
	 ...
	 * @return
	 */
	private String getServerConfig(EsbConsoleService service) {
		if (null == service) {
			return null;
		}
		Map<String, Object> context = new HashMap<String, Object>();
		...
		InputStream in = null;
		try {
			in = EsbSAMServiceManager.class.getResourceAsStream("/META-INF/templates/ESB/console/server.xml"); //$NON-NLS-1$
	...
	private static String getDriver(String dbType) {
		dbType = null == dbType ? EsbConsoleService.DATABASE_MySQL : dbType;
