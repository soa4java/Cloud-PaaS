...
function _dohelp() {
	echo "Usage:"
	echo "./stop.sh -reqId 0 -srvInstId 10000 -srvDefName RedisSentinel -clusterName 10000"   
	echo "-h) print help"
...
function _doexecute() {
	...
	if [ -f ${pid_file} ]; then
		...
		if [ ! -z ${pid} ] && [ `ps -p ${pid} | grep -v PID | wc -l` -eq 0 ]; then
			rm -f ${pid_file}
			_success
			_return "Redis Sentinel ${INSTANCE_ID} has been shut down."
			exit 0
		fi
	fi
	...
	if [ ! -z ${pid} ]; then
		...
		while [ ${COUNT} -lt ${TIMEOUT} ]; do
			if [ `ps -p ${pid} | grep -v PID | wc -l` -eq 0 ]; then
				rm -f ${pid_file}
				_return "Redis Sentinel ${INSTANCE_ID} shut down at ${COUNT} seconds."
				_success
				exit 0
			else
				COUNT=`expr ${COUNT} + 1`
		...
		kill -9 ${pid}
		_return "Redis Sentinel ${INSTANCE_ID} force shut down by kill -9."
	fi
	
	_success "Redis Sentinel ${INSTANCE_ID} shut down."
	exit 0
}
