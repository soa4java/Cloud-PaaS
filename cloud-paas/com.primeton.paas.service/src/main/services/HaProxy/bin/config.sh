...
function _doexecute() {
	...
	echo "	clitimeout 	60000" >> "${config}"
	echo "	srvtimeout 	60000" >> "${config}"
	...
	if [ "${protocal}" == "http" ]; then
		echo "	option httpchk HEAD $health_uri" >> "${config}"
    fi
    
	
	back_urls=(`echo $back_urls | tr ";" ","`)
	url_array=(`echo $back_urls | tr "," "\n"`)
	
	index=1
	for url in ${url_array[@]}; do
	 		echo "	server server_$index $url check inter ${inter}  rise 1 fall 2" >> "${config}"
	        (( index ++ ))
	done
