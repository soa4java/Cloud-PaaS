...
package_home=${BIN_HOME_PATH}/EsbSAM/packages/java
tomcat_package=${package_home}/apache-tomcat-6.0.44.tar.gz
sam_war=${package_home}/sam.war

program_home=${PROGRAME_HOME_PATH}/EsbSAM
...
function _doexecute() {
	...
	# deploy
	mkdir -p ${program_home}/apache-tomcat-6.0.44/webapps/sam
	
	unzip ${sam_war} -d ${program_home}/apache-tomcat-6.0.44/webapps/sam
	...
	_success
	_return "[`date`] [INFO] Install EsbSAM service success."
	exit 0
}
