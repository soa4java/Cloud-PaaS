...
function _dohelp() {
	echo "Usage:"
	echo "./stop.sh -reqId 10001 -srvDefName EsbConsole -srvInstId 20001 -clusterName 20001"
	echo "-h) print help"
...
function _doexecute() {
	...
	if [ -f ${CATALINA_PID} ]; then
		...
		if [ ! -z ${PID} ] && [ `ps -p ${PID} | grep -v PID | wc -l` -eq 0 ]; then
			_success
			_return "EsbConsole ${INSTANCE_ID} has been shut down."
			exit 0
		fi
	fi
	...
	if [ ! -z ${PID} ]; then
		...
		while [ ${COUNT} -lt ${TIMEOUT} ]; do
			if [ `ps -p ${PID} | grep -v PID | wc -l` -eq 0 ]; then
				_return "EsbConsole ${INSTANCE_ID} shut down at ${COUNT} seconds."
				_success
				exit 0
			else
				COUNT=`expr ${COUNT} + 1`
		...
		kill -9 ${PID}
		_return "EsbConsole ${INSTANCE_ID} force shut down by kill -9."
	fi
