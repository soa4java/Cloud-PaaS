...
function _dohelp() {
	echo "Usage:"
	echo "./start.sh -reqId 10001 -srvDefName EsbConsole -srvInstId 20001 -clusterName 20001 -port 8020 -shutdownPort 8021 -ajpPort 8022"
	echo "./start.sh -reqId 10002 -srvDefName EsbConsole -srvInstId 20001 -clusterName 20001 -port 8020 -shutdownPort 8021 -ajpPort 8022 -ip ${ip} -minMemory 128 -maxMemory 256 -minPermSize 64 -maxPermSize 128"
...
function _doexecute() {
	...
	# Tomcat
	TOMCAT_HOME=${PROGRAME_HOME_PATH}/EsbConsole/apache-tomcat-6.0.44
	CONFIG_DIR=${PROGRAME_HOME_PATH}/EsbConsole/app_configs
	
	if [ ! -d ${CONFIG_DIR}/governor ]; then
		mkdir -p ${CONFIG_DIR}/governor
	fi
	STARTUP_CONF=${CONFIG_DIR}/governor/startup.conf
	if [ -f ${STARTUP_CONF} ]; then
		rm -f ${STARTUP_CONF}
	fi
	echo "# Auto generate by EsbConsole/bin/start.sh" > ${STARTUP_CONF}
	...
	if [ -f ${CATALINA_PID} ]; then
		...
		if [ ! -z ${PID} ] && [ `ps -p ${PID} | grep -v PID | wc -l` -gt 0 ]; then
			_success
			_return "EsbConsole ${INSTANCE_ID} has been started."
			exit 0
		fi
	fi
	...
	if [ ! -z ${PID} ] && [ `ps -p ${PID} | grep -v PID | wc -l` -gt 0 ]; then
		_success
		_return "EsbConsole ${INSTANCE_ID} start successfully, pid=${PID}."
		exit 0
	else
		_fail
		_error "EsbConsole ${INSTANCE_ID} failed to start."
		exit 0
	fi
}
