...
function stop() {
	if [ -f ${f_keepalived_pid} ]; then
		...
		if [ `ps -p ${pid} | grep -v PID | wc -l` -eq 1 ]; then
			logger "[`date`] Begin stop keepalived."
			kill ${pid}
			logger "[`date`] Finish stop keepalived."
		fi
	fi
}

# monitor nginx process
if [ `ps -p ${nginx_pid} | grep -v PID | wc -l` -eq 0 ]; then
	logger "[WARN] Nginx ${nginx_pid} not found."
	stop
else
	logger "[INFO] Nginx running, pid=${nginx_pid}."
fi
