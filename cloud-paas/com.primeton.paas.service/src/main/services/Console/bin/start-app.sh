...
JAVA_OPTS=" -Xms256m -Xmx512m -XX:MaxPermSize=256m -XX:-UseGCOverheadLimit -XX:+UseParallelGC -XX:ParallelGCThreads=4 -XX:+UseParallelOldGC -Djava.net.preferIPv4Stack=true -Djava.awt.headless=true -Djetty.home=${JETTY_HOME} -Djetty.port=${JETTY_PORT} -XX:+HeapDumpOnOutOfMemoryError -Duser.timezone=GMT+08"
# language and region
...
JETTY_OPTS="-jar ${JETTY_HOME}/start.jar jetty.port=${JETTY_PORT} STOP.PORT=3341 STOP.KEY=3331 "
...
if [ ! -z ${PID} ]; then
	if [ `ps -p ${PID} |grep -v PID | wc -l` -gt 0 ]; then
		echo "##############################################################################"
		echo "[INFO] Primeton PAAS console-app has been started. PID = ${PID}"
		echo "###############################################################################"
		exit 0
	fi
fi
...
if [ `ps -ef | grep ${keyword1} | grep ${keyword2} | grep ${keyword3} | grep ${keyword4} | grep -v grep | wc -l` -gt 0 ]; then
	echo "#############################################################"
	echo "[INFO] Primeton PAAS console-app has been started."
	echo "#############################################################"
	exit 0
fi
...
echo "##################################################################################"
echo "Primeton PAAS console-app start success, pid ${PID}, listen *:${JETTY_PORT}."
echo "##################################################################################"